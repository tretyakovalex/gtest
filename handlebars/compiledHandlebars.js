...
async function generatePdf(clientInvoiceData) {
    try {
        ...
        let pdf_directory = path.join(__dirname, 'gsa-invoices');
        const file_name = await getFilePath(pdf_directory, clientInvoiceData.invoice_file_name);
        ...
        const content = await compile('gsaClientInvoiceTemplate', modifiedClientInvoiceData);  // Pass the transformed purchase data
        
        // Compile to HTML file
        const htmlFilePath = path.join(__dirname, 'compiled-gsaClientInvoiceTemplate.html');
        ...
        const pdfPath = path.join(__dirname, 'gsa-invoices', file_name);
        await page.pdf({
            path: pdfPath,
            format: 'A4',
